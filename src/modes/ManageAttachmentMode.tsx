...
import {
  ...
  type FileOperation,
  type AttachmentUploadMethod,
  type AttachmentDownloadMethod,
...
import {
  serializeManageAttachment,
  deserializeManageAttachment,
  hashState,
  type SavedRequest,
  type SerializedManageAttachmentState,
...
export function ManageAttachmentMode({ themeMode }: { themeMode: ThemeMode }) {
  ...
  const set = <K extends keyof ManageAttachmentState>(
    k: K,
    v: ManageAttachmentState[K],
    dirtyId?: string,
  ) => {
    setState((s) => ({ ...s, [k]: v }));
  ...
  const advisories = useMemo<Advisory[]>(() => {
    ...
    return [
      adv.warn(
        ...
        <span>
          PPTB's <code>dataverseAPI</code> doesn't expose the attachment / annotation message
          endpoints. Build the request here, then copy a snippet from the <strong>Code</strong> tab
          (fetch / curl / C# / PowerShell) and run it from there. Execute will return{' '}
  ...
  const sections = [
    ...
    {
      id: 'parent',
      label: state.target === 'attachment' ? 'Parent email' : 'Annotation id',
      meta:
        state.target === 'attachment'
          ? state.parentActivityId
            ? state.parentActivityId.slice(0, 8)
            : 'unset'
          : state.recordId
            ? state.recordId.slice(0, 8)
            : 'unset',
      items: [
        {
          id: 'parent',
          ...
          label:
            state.target === 'attachment'
              ...
              : 'Annotation id + optional parent',
          badge: (state.target === 'attachment' ? !!state.parentActivityId : !!state.recordId)
            ? '✓'
            : null,
          badgeAppearance: 'ghost' as const,
  ...
  const cards: { id: FileOperation; label: string; sub: string; tone: 'brand' | 'danger' }[] = [
    {
      id: 'upload',
      label: 'Upload',
      sub: 'Inline base 64 (≲4 MB) or Initialize/UploadBlock/Commit messages.',
      tone: 'brand',
    },
    {
      id: 'download',
      label: 'Download',
      sub: 'GET /<set>(<id>)/{body|documentbody}/$value or Initialize/DownloadBlock messages.',
      tone: 'brand',
    },
    {
      id: 'delete',
      label: 'Delete',
      sub: 'DELETE /<set>(<id>) — removes the entire attachment / note record.',
      tone: 'danger',
    },
  ];
  return (
    <div>
      <PaneHead
        ...
        title="Operation"
        sub="What to do with the attachment or annotation."
        group="binary"
      />
      <div
...
function TargetPicker({
  ...
  onChange: (t: AttachmentTarget) => void;
}) {
  return (
    <div>
      <PaneHead
        ...
        title="Target type"
        sub="Pick the record type — drives the body column, message names, and parent reference shape."
        group="binary"
      />
            ...
            <Radio
              ...
              label={
                <span>
                  ...
                  >
                    Hangs off an email activity. Upload requires{' '}
                    <code>objectid_email@odata.bind</code>. Server generates the
            ...
            <Radio
              ...
              label={
                <span>
                  ...
                  >
                    Hangs off any HasNotes=true entity. Caller MUST generate the annotationid GUID
                    for Initialize/Commit messages.
                  </Caption1>
                </span>
              }
            />
  ...
  return (
    <div>
      <PaneHead
        ...
        title={
          p.target === 'attachment'
            ? 'Parent email & record reference'
            : 'Annotation id & parent reference'
        }
        sub={
          p.target === 'attachment'
      ...
      <div style={{ maxWidth: 760, display: 'flex', flexDirection: 'column', gap: 14 }}>
        {p.target === 'attachment' ? (
          <>
            <Field
              label="Parent email activityid (GUID)"
              hint="Bound via objectid_email@odata.bind: emails(<id>) on the Initialize/Commit Target."
            >
              <Input
                value={p.parentActivityId ?? ''}
                onChange={(_, d) => p.onParentActivityIdChange(d.value || null)}
            ...
            <Field label="Existing activitymimeattachmentid (update/download/delete only)">
              <Input
                value={p.recordId ?? ''}
                onChange={(_, d) => p.onRecordIdChange(d.value || null)}
                placeholder="leave empty for create"
              />
            </Field>
            <Field
              label="Subject"
              hint="Optional. Stored on the attachment record alongside the binary."
            >
              <Input
                value={p.subject}
                onChange={(_, d) => p.onSubjectChange(d.value)}
                placeholder="e.g. Sample attached 25mb.pdf"
              />
            </Field>
          </>
        ) : (
          <>
            <Field
              label="Annotation id (GUID)"
              hint="Caller-generated. Initialize / Commit will NOT auto-assign one for these messages."
            >
              <Input
                value={p.recordId ?? ''}
                onChange={(_, d) => p.onRecordIdChange(d.value || null)}
            ...
            <Field
              label="Parent entity logical name (optional)"
              hint="Annotations can hang off any HasNotes=true entity. Leave blank for unparented notes."
            >
              <Input
                value={p.parentEntityLogical ?? ''}
                onChange={(_, d) => p.onParentEntityLogicalChange(d.value || null)}
                placeholder="account / contact / opportunity / …"
              />
            </Field>
            <Field label="Parent entity id (GUID)">
              <Input
                value={p.parentEntityId ?? ''}
                onChange={(_, d) => p.onParentEntityIdChange(d.value || null)}
                placeholder="00000000-0000-0000-0000-000000000000"
              />
            </Field>
            <Field
              label="Notetext"
              hint="Free-text body of the note — separate from documentbody (which carries the binary)."
            >
              <Textarea
                value={p.noteText}
                onChange={(_, d) => p.onNoteTextChange(d.value)}
                placeholder="Please see attached…"
              />
            </Field>
          </>
        )}
...
function UploadMethodPicker({
  ...
  value: AttachmentUploadMethod;
  onChange: (m: AttachmentUploadMethod) => void;
}) {
  return (
    <div>
      <PaneHead
        ...
        title="Upload method"
        sub="Per attachment-annotation-files — two supported approaches."
        group="binary"
      />
      <div style={{ maxWidth: 760, display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <Radio
              ...
              label={
                <span>
                  ...
                  <code style={{ fontFamily: tokens.fontFamilyMonospace, fontSize: 11 }}>
                    POST / PATCH with body column set
                  </code>
                  <Caption1
                    style={{
                      display: 'block',
                  ...
                  >
                    Works for files ≲ 4 MB (Organization.MaxUploadFileSize default 5 MB). One
                    round-trip. Body column = base 64 string in the JSON body.
                  </Caption1>
                </span>
              }
            />
            <Radio
              ...
              label={
                <span>
                  ...
                  >
                    Three OData actions. Up to 128 MB. UploadBlock is shared with file columns.
                    Commit returns FileSizeInBytes + record id.
                  </Caption1>
                </span>
              }
            />
        ...
        {value.kind === 'dataverse-messages' && (
          <Field
            ...
            hint="Defaults to 4 MB. BlockData must be ≤ 4 MB before base 64 encoding."
          >
            <SpinButton
...
function DownloadMethodPicker({
  ...
  value: AttachmentDownloadMethod;
  onChange: (m: AttachmentDownloadMethod) => void;
}) {
  return (
    <div>
      <PaneHead
        ...
        sub="Per attachment-annotation-files — two supported approaches. No ranged download / SAS URL (those are file-column-only)."
        group="binary"
      />
      <div style={{ maxWidth: 760, display: 'flex', flexDirection: 'column', gap: 14 }}>
        <Field label="Approach">
            ...
            <Radio
              ...
              label={
                <span>
                  ...
                  >
                    Returns the binary as base 64 text/plain. No size / filename / mimetype headers
                    — query those via $select on filename / mimetype.
                  </Caption1>
                </span>
              }
            />
            <Radio
              ...
              label={
                <span>
                  ...
                  >
                    Returns FileName + FileSizeInBytes + FileContinuationToken on Initialize. Then
                    loop DownloadBlock with Offset stepping by BlockLength.
                  </Caption1>
                </span>
              }
            />
        ...
        {value.kind === 'dataverse-messages' && (
          <Field
            label="Block size (bytes)"
            hint="Defaults to 4 MB. Can stay constant — final block may be smaller and only those bytes are returned."
          >
            <SpinButton
              value={value.blockSize}
              min={64 * 1024}
