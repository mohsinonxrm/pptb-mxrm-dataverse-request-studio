...
import {
  RadioGroup, Radio, Caption1, Badge, Field, tokens, mergeClasses,
...
import {
  serializeManageImage, deserializeManageImage, hashState,
  type SavedRequest, type SerializedManageImageState,
...
type RootClauseId = 'operation' | 'target' | 'column' | 'source' | 'preview' | 'options' | 'pipeline' | 'headers';
...
export function ManageImageMode({ themeMode }: { themeMode: ThemeMode }) {
  ...
  const imageCol = useMemo(
    () => (tbl?.columns ?? []).find((c): c is ImageColumnMeta => c.attributeType === 'Image' && c.logicalName === state.imageColumn),
    [tbl, state.imageColumn],
  );

  const markDirty = (id: string) => setState(s => { const d = new Set(s.dirty); d.add(id); return { ...s, dirty: d }; });
  const set = <K extends keyof ManageImageState>(k: K, v: ManageImageState[K], dirtyId?: string) => {
    setState(s => ({ ...s, [k]: v }));
  ...
  const disabledReason =
    ...
    !state.imageColumn ? 'Pick an image column.' :
    state.operation === 'upload' && !state.bodyBase64 && state.source.kind === 'file' ? 'Pick an image file.' :
    state.operation === 'download' && state.downloadSize === 'full' && !imageCol?.canStoreFullImage
  ...
  const onExecute = async () => {
    ...
    setTab('results');
    setRecents(rs => [{
      id: `r-${Date.now()}`, modeId: 'manage-image',
  ...
  const advisories = useMemo<Advisory[]>(() => {
    ...
    return [adv.warn(
      ...
      <span>
        PPTB's <code>dataverseAPI</code> doesn't expose image column endpoints.
  ...
  const sections = [
    ...
    },
    {
      ...
      items: [
        ...
        {
          id: 'column',
          icon: Image20Regular, iconFilled: Image20Filled,
          label: state.imageColumn || 'Pick an image column',
          code: !!state.imageColumn,
          badge: imageCol?.isPrimaryImage ? 'primary' : imageCol?.canStoreFullImage ? 'full' : 'thumb',
    ...
    ...(imageCol ? [{
      ...
      items: [
        { id: 'd1', icon: Image20Regular, label: `${imageCol.logicalName}id`, code: true, badge: 'guid', badgeAppearance: 'ghost' as const },
        { id: 'd2', icon: Image20Regular, label: `${imageCol.logicalName}_Timestamp`, code: true, badge: 'bigint', badgeAppearance: 'ghost' as const },
        { id: 'd3', icon: Image20Regular, label: `${imageCol.logicalName}_URL`, code: true, badge: 'url', badgeAppearance: 'ghost' as const },
      ],
    }] : []),
    ...
    ...(state.operation === 'download' ? [{
      ...
      items: [{
        ...
        label: 'Download size',
        badge: state.downloadSize, badgeAppearance: 'tint' as const, badgeColor: 'brand' as const,
  ...
  switch (root) {
    case 'operation':
      pane = <OperationPicker value={state.operation} onChange={(op) => set('operation', op, 'operation')} />;
      break;
    case 'target':
      pane = (
        <TargetEditor
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <BinaryPipelineCard
            ...
            sizeAdvisory={state.operation === 'upload'
              ? checkUploadSize(approxBytesFromBase64(state.bodyBase64), imageCol?.maxSizeInKB ? imageCol.maxSizeInKB * 1024 : undefined, 'image')
              : null}
          />
        </div>
      );
  ...
  const codeInputs = {
    ...
    built,
    headers: headersMap,
    body: pipeline[0] && typeof pipeline[0].body === 'object' ? (pipeline[0].body as Record<string, unknown>) : undefined,
...
function PreviewCard({
  state, imageCol, initials, setSource, setBodyBase64, setFileName, setMimeType,
}: {
  ...
  return (
    <div className={mergeClasses(s.inlineCard)} style={{ padding: 14, maxWidth: 880 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
        ...
        <Badge appearance="tint" color="brand">{imageCol?.isPrimaryImage ? 'primary image' : 'custom image column'}</Badge>
        {isThumb && <Badge appearance="ghost">144×144 only</Badge>}
        {imageCol?.canStoreFullImage && <Badge appearance="tint" color="success">full-size enabled</Badge>}
      </div>
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
        ...
        <div>
          <Caption1 style={{ color: tokens.colorNeutralForeground3, display: 'block', marginBottom: 4 }}>
            {state.operation === 'upload' ? 'Preview' : state.downloadSize === 'full' ? 'Full-size' : 'Thumbnail'} · 144×144
          </Caption1>
        ...
        {state.operation !== 'upload' && (
          <div style={{ flex: 1, fontSize: 11, color: tokens.colorNeutralForeground3, lineHeight: 1.6 }}>
            {state.operation === 'download' && (
              <>
                <strong style={{ color: tokens.colorNeutralForeground2 }}>Download:</strong> server returns the {state.downloadSize === 'full' ? 'original full-size' : 'auto-cropped 144×144 thumbnail'} as raw bytes.
                The Content-Type matches the stored image's MIME type (one of image/png · image/jpeg · image/gif · image/bmp · image/tiff).
              </>
            )}
            {state.operation === 'delete' && (
              <>
                <strong style={{ color: tokens.colorNeutralForeground2 }}>Delete:</strong> clears the image data + companion columns
                (<code style={{ fontFamily: tokens.fontFamilyMonospace }}>{imageCol?.logicalName}id</code>,{' '}
                <code style={{ fontFamily: tokens.fontFamilyMonospace }}>_Timestamp</code>,{' '}
                <code style={{ fontFamily: tokens.fontFamilyMonospace }}>_URL</code>).
                Equivalent forms: PATCH set null, PUT /col with body <code>{'{"value":null}'}</code>, or DELETE /col (shown).
              </>
            )}
          </div>
        )}
...
function OperationPicker({ value, onChange }: { value: FileOperation; onChange: (op: FileOperation) => void }) {
  ...
  const cards: { id: FileOperation; label: string; sub: string; tone: 'brand' | 'danger' }[] = [
    { id: 'upload',   label: 'Upload',   sub: 'PATCH /<set>(<id>) with body {col: <base64>}.', tone: 'brand' },
    { id: 'download', label: 'Download', sub: 'GET /<set>(<id>)/<col>/$value (?size=full for full).', tone: 'brand' },
    { id: 'delete',   label: 'Delete',   sub: 'DELETE / PATCH null / PUT /col body=null — three equivalents.', tone: 'danger' },
  ];
  return (
    <div>
...
function DownloadOptionsPane({
  downloadSize, setDownloadSize, canStoreFullImage,
}: {
  ...
  return (
    <div>
      <PaneHead icon={Settings20Filled} title="Download options" sub="Image columns can store both thumbnail + full-size when CanStoreFullImage is true." group="binary" />
            ...
            <Radio
              ...
              label={
                <span>
                  ...
                  <code style={{ fontFamily: tokens.fontFamilyMonospace, fontSize: 11 }}>?size=full</code>
                  <Caption1 style={{ display: 'block', color: tokens.colorNeutralForeground3, marginTop: 2 }}>
                    Only available when the column has CanStoreFullImage = true. Server returns 204 if not configured.
                    {!canStoreFullImage && <> · <span style={{ color: tokens.colorPaletteRedForeground1 }}>this column doesn't support full-size</span></>}
                  </Caption1>
                </span>
              }
            />
...
// Operation banner
// ──────────────────────────────────────────────────────────────
function OperationBanner({ state, imageCol }: { state: ManageImageState; imageCol: ImageColumnMeta | undefined }) {
  if (!imageCol) {
    return (
      <MessageBar layout="multiline" intent="info">
        <MessageBarBody>Pick an image column on the left to see operation-specific guidance.</MessageBarBody>
      </MessageBar>
    );
  }
  if (state.operation === 'upload') {
    return (
      <MessageBar layout="multiline" intent="info">
        <MessageBarBody>
          <MessageBarTitle>{imageCol.isPrimaryImage ? 'Primary image — thumbnail only' : 'Custom image column'}</MessageBarTitle>
          {imageCol.isPrimaryImage
            ? <><code>{imageCol.logicalName}</code> is the table's primary image (auto-cropped 144×144). Full-size data is discarded. To store full-resolution images, create a custom image column with <code>CanStoreFullImage = true</code>.</>
            : imageCol.canStoreFullImage
              ? <><code>{imageCol.logicalName}</code> supports full-size (max {((imageCol.maxSizeInKB ?? 10240) / 1024).toFixed(0)} MB). Upload via PATCH; for files &gt; 4 MB use the file-column chunked upload path on the underlying file storage.</>
              : <><code>{imageCol.logicalName}</code> stores thumbnail only (CanStoreFullImage = false). Full-size won't be retained.</>}
        </MessageBarBody>
      </MessageBar>
    );
  }
    ...
    return (
      <MessageBar layout="multiline" intent="info">
        <MessageBarBody>
          ...
          GET /$value returns the {state.downloadSize === 'full' ? 'full-size original' : '144×144 thumbnail'}.
          Image companion columns (<code>{imageCol.logicalName}_URL</code> etc.) let you compose a download URL for browser-side use.
        </MessageBarBody>
      </MessageBar>
    );
  }
  return (
    <MessageBar layout="multiline" intent="warning">
      <MessageBarBody>
        ...
        Clears <code>{imageCol.logicalName}</code> + its companion columns (id / _Timestamp / _URL). Three equivalent paths per docs: DELETE /col (shown), PATCH set null, or PUT /col with body <code>{'{"value":null}'}</code>.
      </MessageBarBody>
    </MessageBar>
  );
}
