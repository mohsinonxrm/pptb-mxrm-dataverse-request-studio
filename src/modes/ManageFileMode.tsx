...
import {
  Field, RadioGroup, Radio, Caption1, Badge, tokens, mergeClasses, SpinButton,
...
import { checkUploadSize, type ManageFileState, type FileOperation, type FileUploadMethod, type FileDownloadMethod } from '../state/binaryState';
...
import {
  serializeManageFile, deserializeManageFile, hashState,
  type SavedRequest, type SerializedManageFileState,
...
type RootClauseId = 'operation' | 'target' | 'column' | 'method' | 'source' | 'pipeline' | 'headers';
...
export function ManageFileMode({ themeMode }: { themeMode: ThemeMode }) {
  ...
  const fileCol = useMemo(
    () => (tbl?.columns ?? []).find((c): c is FileColumnMeta => c.attributeType === 'File' && c.logicalName === state.fileColumn),
  ...
  const disabledReason =
    ...
    !state.recordId ? 'Pick a source record.' :
    !state.fileColumn ? 'Pick a file column.' :
    state.operation === 'upload' && state.source.kind === 'file' && !state.bodyBase64 && state.source.kind === 'file' && !state.fileName
  ...
  const onExecute = async () => {
    ...
    setTab('results');
    setRecents(rs => [{
      id: `r-${Date.now()}`, modeId: 'manage-file',
  ...
  const advisories = useMemo<Advisory[]>(() => {
    ...
    return [adv.warn(
      ...
      <span>
        PPTB's <code>dataverseAPI</code> doesn't expose file column endpoints.
        Build the request here, then copy a snippet from the <strong>Code</strong>{' '}
        tab (fetch / curl / C# / PowerShell) and run it from there. Execute
        will return <code>501 Not Implemented</code>.
      </span>,
      'pipeline',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <BinaryPipelineCard
            ...
            sizeAdvisory={state.operation === 'upload'
              ? checkUploadSize(state.fileSize, fileCol?.maxSizeInKB ? fileCol.maxSizeInKB * 1024 : undefined, 'file')
              : null}
          />
        </div>
      );
...
function OperationPicker({ value, onChange }: { value: FileOperation; onChange: (op: FileOperation) => void }) {
  ...
  const cards: { id: FileOperation; label: string; sub: string; tone: 'brand' | 'danger' }[] = [
    { id: 'upload',   label: 'Upload',   sub: 'PATCH or message-based upload pipeline.',              tone: 'brand' },
    { id: 'download', label: 'Download', sub: 'GET /$value, ranged GET, message pipeline, or SAS URL.', tone: 'brand' },
    { id: 'delete',   label: 'Delete',   sub: 'DELETE /<set>(<id>)/<col> · clears the column value.', tone: 'danger' },
  ];
  return (
    <div>
      <PaneHead icon={ArrowSwap20Filled} title="Operation" sub="What to do with the file column — drives the URL bar method + pipeline shape." group="binary" />
...
// ──────────────────────────────────────────────────────────────
function UploadMethodPicker({ value, onChange }: { value: FileUploadMethod; onChange: (m: FileUploadMethod) => void }) {
  return (
    <div>
      <PaneHead icon={Settings20Filled} title="Upload method" sub="Per file-column-data — three supported approaches." group="binary" />
      <div style={{ maxWidth: 760, display: 'flex', flexDirection: 'column', gap: 14 }}>
        <Field label="Approach">
          <RadioGroup
            ...
            onChange={(_, d) => {
              switch (d.value) {
                case 'single-request':       onChange({ kind: 'single-request' }); break;
                case 'chunked-patch':        onChange({ kind: 'chunked-patch', chunkSize: value.kind === 'chunked-patch' ? value.chunkSize : 2 * 1024 * 1024 }); break;
                case 'dataverse-messages':   onChange({ kind: 'dataverse-messages', blockSize: value.kind === 'dataverse-messages' ? value.blockSize : 4 * 1024 * 1024 }); break;
              }
            }}
          >
            <Radio value="single-request" label={
              <span>
                ...
                <code style={{ fontFamily: tokens.fontFamilyMonospace, fontSize: 11 }}>PATCH /(set)(id)/col</code>
                <Caption1 style={{ display: 'block', color: tokens.colorNeutralForeground3, marginTop: 2 }}>
                  Fastest for files up to ~128 MB. Body = raw bytes; x-ms-file-name header. No init/commit ceremony.
                </Caption1>
              </span>
            }/>
            <Radio value="chunked-patch" label={
              <span>
                <strong>Chunked PATCH</strong>{' '}
                <code style={{ fontFamily: tokens.fontFamilyMonospace, fontSize: 11 }}>x-ms-transfer-mode: chunked</code>
                <Caption1 style={{ display: 'block', color: tokens.colorNeutralForeground3, marginTop: 2 }}>
                  Init PATCH returns Location + x-ms-chunk-size. Each chunk = PATCH with Content-Range. Server commits on final chunk.
                </Caption1>
              </span>
            }/>
            <Radio value="dataverse-messages" label={
              <span>
                ...
                <code style={{ fontFamily: tokens.fontFamilyMonospace, fontSize: 11 }}>InitializeFileBlocksUpload / UploadBlock / CommitFileBlocksUpload</code>
                <Caption1 style={{ display: 'block', color: tokens.colorNeutralForeground3, marginTop: 2 }}>
                  Three OData actions. Up to 10 GB. Returns FileId on commit. Use for files &gt; 128 MB or when you need parallel block uploads.
                </Caption1>
              </span>
            }/>
          </RadioGroup>
        ...
        {value.kind === 'chunked-patch' && (
          <Field label="Chunk size (bytes)" hint="Defaults to 2 MB. Server may suggest a different size via x-ms-chunk-size on init.">
            <SpinButton
              value={value.chunkSize}
              min={64 * 1024}
              max={64 * 1024 * 1024}
              step={1024 * 1024}
              onChange={(_, d) => onChange({ kind: 'chunked-patch', chunkSize: Number(d.value ?? d.displayValue ?? value.chunkSize) })}
            />
        ...
        {value.kind === 'dataverse-messages' && (
          <Field label="Block size (bytes)" hint="Default = 4 MB. The docs cap each UploadBlock at 4 MB; smaller is fine.">
            <SpinButton
...
// ──────────────────────────────────────────────────────────────
function DownloadMethodPicker({ value, onChange }: { value: FileDownloadMethod; onChange: (m: FileDownloadMethod) => void }) {
  return (
    <div>
      <PaneHead icon={Settings20Filled} title="Download method" sub="Per file-column-data + getfilesasurl — four supported approaches." group="binary" />
      <div style={{ maxWidth: 760, display: 'flex', flexDirection: 'column', gap: 14 }}>
        <Field label="Approach">
          <RadioGroup
            value={value.kind}
            onChange={(_, d) => {
              switch (d.value) {
                case 'single-request':       onChange({ kind: 'single-request' }); break;
                case 'ranged':               onChange({ kind: 'ranged', rangeStart: 0, rangeEnd: 4 * 1024 * 1024 - 1 }); break;
                case 'dataverse-messages':   onChange({ kind: 'dataverse-messages', blockSize: 4 * 1024 * 1024 }); break;
                case 'sas-url':              onChange({ kind: 'sas-url' }); break;
              }
            }}
          >
            <Radio value="single-request" label={
              <span><strong>GET /$value</strong>
              <Caption1 style={{ display: 'block', color: tokens.colorNeutralForeground3, marginTop: 2 }}>
                Stream the entire file in one request. Returns raw bytes. Easy but not great for huge files.
              </Caption1></span>
            }/>
            <Radio value="ranged" label={
              <span><strong>Ranged GET</strong>{' '}
                <code style={{ fontFamily: tokens.fontFamilyMonospace, fontSize: 11 }}>Range: bytes=A-B</code>
                <Caption1 style={{ display: 'block', color: tokens.colorNeutralForeground3, marginTop: 2 }}>
            ...
            <Radio value="dataverse-messages" label={
              <span><strong>Dataverse messages</strong>{' '}
                <code style={{ fontFamily: tokens.fontFamilyMonospace, fontSize: 11 }}>InitializeFileBlocksDownload + DownloadBlock</code>
                <Caption1 style={{ display: 'block', color: tokens.colorNeutralForeground3, marginTop: 2 }}>
                  Block-by-block download. Returns FileName + FileSizeInBytes + FileContinuationToken from init.
                </Caption1>
              </span>
            }/>
            <Radio value="sas-url" label={
              <span><strong>GetFileSasUrl</strong>{' '}
                <code style={{ fontFamily: tokens.fontFamilyMonospace, fontSize: 11 }}>GET /GetFileSasUrl(…)</code>
                <Caption1 style={{ display: 'block', color: tokens.colorNeutralForeground3, marginTop: 2 }}>
                  Returns a 1-hour SAS URL that anyone can use to download directly from Azure Storage. Skips Web API roundtrip on the actual download.
                </Caption1>
              </span>
            }/>
          </RadioGroup>
          ...
          <div style={{ display: 'flex', gap: 10, maxWidth: 480 }}>
            <Field label="Range start (bytes)">
              <SpinButton
                ...
                min={0}
                onChange={(_, d) => onChange({ kind: 'ranged', rangeStart: Number(d.value ?? d.displayValue ?? 0), rangeEnd: value.rangeEnd })}
              />
            </Field>
            <Field label="Range end (bytes)">
              <SpinButton
                ...
                min={0}
                onChange={(_, d) => onChange({ kind: 'ranged', rangeStart: value.rangeStart, rangeEnd: Number(d.value ?? d.displayValue ?? 0) })}
              />
            </Field>
          </div>
        )}
          ...
          <MessageBar layout="multiline" intent="info">
            <MessageBarBody>
              ...
              <ul style={{ margin: '4px 0 0', paddingLeft: 18, lineHeight: 1.6 }}>
                ...
                <li>Does NOT work for environments using BYOK encryption — migrate to CMK first.</li>
                <li>Caller must have access to the record; the resulting URL grants anonymous access for 1 hour.</li>
              </ul>
            </MessageBarBody>
          </MessageBar>
        )}
...
// ──────────────────────────────────────────────────────────────
function OperationBanner({ state, fileCol }: { state: ManageFileState; fileCol: FileColumnMeta | undefined }) {
    ...
    return (
      <MessageBar layout="multiline" intent={above128mb ? 'warning' : 'info'}>
        <MessageBarBody>
          ...
          {above128mb
            ? <>File is over <strong>128 MB</strong> — single-request PATCH won't work. Use the Dataverse messages path which supports up to 10 GB.</>
            : above4mb
              ? <>File is over <strong>4 MB</strong> — chunked PATCH (or Dataverse messages) is recommended; single-request still works up to 128 MB.</>
    ...
    return (
      <MessageBar layout="multiline" intent="info">
        <MessageBarBody>
          ...
          Retrieving a file column returns the <strong>file id</strong> (not bytes) — use the operations below to fetch actual content. GET /$value is the simplest; SAS URL skips the Web API roundtrip when an anonymous browser-side download is fine.
        </MessageBarBody>
      </MessageBar>
    );
  }
  return (
    <MessageBar layout="multiline" intent="warning">
      <MessageBarBody>
        ...
        Clears the file column value + the <code>_name</code> companion. The underlying file is removed by Dataverse. Alternative equivalent paths: PATCH the column to null, or call the <code>DeleteFile</code> action with the column's current FileId.
      </MessageBarBody>
    </MessageBar>
  );
}
