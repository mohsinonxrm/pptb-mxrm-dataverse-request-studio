...
import { defaultBypassOptions, type CreateState, type LookupFieldValue, type CreateFieldValue } from '../state/writeState';
...
import {
  serializeCreate, deserializeCreate, hashState,
  type SavedRequest, type SerializedCreateState,
...
export function CreateMode({ themeMode }: { themeMode: ThemeMode }) {
  ...
  const advisories = useMemo(
    () => detectBypassAdvisories(state.bypass, 'bypass'),
    [state.bypass],
  );
  ...
  const sections = [
    ...
    },
    {
      ...
      items: [{
        ...
        label: 'Field set',
        badge: missingRequired.length > 0
