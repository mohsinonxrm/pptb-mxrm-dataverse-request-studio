...
import { totalExpandCount, MAX_EXPANDS_PER_QUERY } from '../editors/expand/expandTree';
...
export function validateRequest(input: ValidateRequestInput): Advisory[] {
  ...
  if (input.expand && input.expand.length > 0) {
    ...
    const hasFlatCollection = input.expand.some((it) => {
      ...
      return (
        nav && nav.cardinality !== 'ManyToOne' && (!it.nestedExpand || it.nestedExpand.length === 0)
      );
    });
    if (hasFlatCollection) {
      out.push(
        adv.info(
          'expand-flat-collection',
          'validation',
          'Flat collection expand can return up to 5,000 rows per parent.',
          <>
            Collections without a nested <code>$expand</code> aren't paginated. Add a nested{' '}
            <code>$expand</code> or use <code>Prefer: odata.maxpagesize</code> to control the size.
          </>,
          'expand',
  ...
  if (input.apply?.enabled) {
    ...
    if (input.orderby && input.orderby.length > 0) {
      out.push(
        adv.err(
          'apply-orderby-conflict',
          'validation',
          '`$orderby` on aggregate output is not supported.',
          <>
            Dataverse rejects <code>$orderby</code> on the alias columns produced by{' '}
            <code>$apply</code> with{' '}
            <em>"The query node SingleValueOpenPropertyAccess is not supported."</em> Remove the{' '}
            <code>$orderby</code> entries, or disable <code>$apply</code>.
          </>,
          'orderby',
    ...
    for (const g of apply.groupby) {
      ...
      if (segs.length === 1) {
        ...
        if (
          ...
          out.push(
            adv.err(
              `apply-groupby-lookup-${g}`,
              'validation',
              `\`groupby\` on bare lookup \`${g}\` is rejected.`,
              <>
                Dataverse rejects both <code>{g}</code> and <code>_{g}_value</code> as group-by
                columns. Drill into the lookup and pick a scalar (e.g.{' '}
                <code>{g}/&lt;related-attr&gt;</code>).
              </>,
              'apply',
              'https://learn.microsoft.com/en-us/power-apps/developer/data-platform/webapi/query/aggregate-data',
            ),
    ...
    for (const a of apply.aggregates) {
      if (!a.alias.trim()) {
        out.push(
          adv.err(
            `apply-agg-empty-alias-${Math.random().toString(36).slice(2)}`,
            'validation',
...
function validateFilterGroup(
  ...
  if (group.negated && groupHasDvFunctionDescendant(group)) {
    out.push(
      adv.err(
        `group-not-with-dvfn-${group.id}`,
        'validation',
        'Negated group contains a Dataverse function — will be rejected.',
        <>
          Dataverse rejects <code>not (…Microsoft.Dynamics.CRM.X(…)…)</code> with HTTP&nbsp;405 even
          when wrapped in parens (test G.9). Move the function out of the negated group, or use the
          explicit <code>Not*</code> sibling (e.g. <code>NotIn</code>, <code>NotBetween</code>,{' '}
          <code>NotEqualUserId</code>, <code>DoesNotContainValues</code>) and drop the group-NOT.
        </>,
        'filter',
      ),
    );
...
function validateFilterNode(
  ...
  if (node.type === 'function') {
    ...
    if (node.col && node.col.includes('/') && !node.col.startsWith((lambdaAlias ?? '') + '/')) {
      ...
      const stripped =
        lambdaAlias && node.col.startsWith(lambdaAlias + '/')
          ? node.col.slice(lambdaAlias.length + 1)
          : node.col;
      if (stripped.includes('/')) {
        out.push(
          adv.err(
            `fn-nav-path-${node.op}-${node.id}`,
            'validation',
    ...
    if (op?.category === 'hierarchy') {
      out.push(
        adv.info(
          `fn-hierarchy-${node.op}-${node.id}`,
          'validation',
          `\`${node.op}\` needs a hierarchical relationship on \`${parentTable.logicalName}\`.`,
          <>
            Hierarchy functions only work on entities configured with a hierarchical relationship
            (e.g. <code>account.parentaccountid</code>). If your entity isn\'t configured, execute
            will fail with <code>0x80047020 "doesn't have a hierarchical relationship"</code>.
          </>,
          'filter',
        ),
      );
