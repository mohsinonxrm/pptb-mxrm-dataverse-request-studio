...
import { Badge, Caption1, Tooltip, Button, tokens, mergeClasses, MessageBar, MessageBarBody, MessageBarTitle } from '@fluentui/react-components';
...
export function BinaryPipelineCard({ steps, title = 'Pipeline', sub, sizeAdvisory }: BinaryPipelineCardProps) {
  ...
  return (
    <div className={mergeClasses(s.inlineCard)} style={{ padding: 12, maxWidth: 1040 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
        ...
        <Badge appearance="ghost">{steps.length} request{steps.length === 1 ? '' : 's'}</Badge>
        {sub && <Caption1 style={{ color: tokens.colorNeutralForeground3, marginLeft: 6 }}>{sub}</Caption1>}
      </div>

      {sizeAdvisory && (
        <div style={{ marginBottom: 10 }}>
          <MessageBar
            intent={sizeAdvisory.level === 'error' ? 'error' : sizeAdvisory.level === 'warn' ? 'warning' : 'success'}
            layout="multiline"
          >
            <MessageBarBody>
      ...
      {steps.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {steps.map(step => <PipelineStepRow key={step.n} step={step} />)}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{
      ...
      {(Object.keys(step.headers).length > 0 || step.body || step.detail) && (
        <div style={{ paddingLeft: 34, marginTop: 8, display: 'flex', flexDirection: 'column', gap: 4, fontSize: 11, color: tokens.colorNeutralForeground3 }}>
          {Object.keys(step.headers).length > 0 && (
            <div>
              <strong style={{ color: tokens.colorNeutralForeground2 }}>Headers</strong>
              <div style={{ marginTop: 2, fontFamily: tokens.fontFamilyMonospace, fontSize: 10, lineHeight: 1.55 }}>
