...
import {
  Caption1, MessageBar, MessageBarBody, Tooltip, Button, Badge,
  Subtitle2, mergeClasses, tokens, Combobox, Option, Field,
} from '@fluentui/react-components';
import {
  BranchFork20Filled, BranchFork20Regular, Add20Regular, Delete20Regular,
  ArrowEnter20Regular,
...
import {
  availableNavsAt, isCollectionExpand, totalExpandCount, maxExpandDepth, MAX_EXPANDS_PER_QUERY,
  hasAnyNestedExpand,
} from './expandTree';
...
export function ExpandOverview({
  parentEntity, siblings, rootExpand, breadcrumb, parentCardinality,
  onAdd, onPickExisting, onRemove, group = 'read',
}: ExpandOverviewProps) {
  ...
  const hasFlatCollection = !queryHasNestedExpand && siblings.some(it =>
    isCollectionExpand(parentEntity, it),
  );
    ...
    <div>
      ...
      {capExceeded && (
        <MessageBar layout="multiline" intent="error" style={{ marginBottom: 12 }}>
          <MessageBarBody>
            <strong>{totalNavs} of {MAX_EXPANDS_PER_QUERY} expand slots used.</strong> Dataverse rejects any query with more than {MAX_EXPANDS_PER_QUERY} <code>$expand</code> options total — remove some before adding more.
          </MessageBarBody>
        </MessageBar>
      )}
      {!capExceeded && totalNavs >= MAX_EXPANDS_PER_QUERY - 3 && (
        <MessageBar layout="multiline" intent="warning" style={{ marginBottom: 12 }}>
          <MessageBarBody>
            <strong>{totalNavs} of {MAX_EXPANDS_PER_QUERY} expand slots used.</strong> Each expand is a server-side join — keep this list lean.
          </MessageBarBody>
        </MessageBar>
      )}
      {depth > 3 && (
        <MessageBar layout="multiline" intent="warning" style={{ marginBottom: 12 }}>
          <MessageBarBody>
            Expand depth is <strong>{depth}</strong>. Every nested expand is another join — keep depth shallow for performance.
          </MessageBarBody>
        </MessageBar>
      )}
      {hasFlatCollection && !isNested && (
        <MessageBar layout="multiline" intent="info" style={{ marginBottom: 12 }}>
          <MessageBarBody>
            Collections without a nested <code>$expand</code> aren't paginated — Dataverse can return up to <strong>5,000 related rows per parent record</strong>. Add a nested <code>$expand</code> or a top-level <code>Prefer: odata.maxpagesize</code> to control the size.
          </MessageBarBody>
        </MessageBar>
      )}
      {parentIsNN && (
        <MessageBar layout="multiline" intent="error" style={{ marginBottom: 12 }}>
          <MessageBarBody>
            The enclosing expand is a <strong>many-to-many (N:N)</strong> relationship. <strong>Dataverse rejects any nested $expand inside an N:N</strong> with:{' '}
            <em>"The navigation property '…' cannot be expanded. Only many-to-one relationships are supported for nested expansion."</em>
          </MessageBarBody>
        </MessageBar>
      )}
      {parentIsCollection && (
        <MessageBar layout="multiline" intent="info" style={{ marginBottom: 12 }}>
          <MessageBarBody>
            ...
            <strong>many-to-one (N:1)</strong> or <strong>one-to-many (1:N)</strong> relationships here.{' '}
            <strong>N:N navs are hidden</strong> — Dataverse rejects N:N expansion anywhere in a query that uses nested $expand.{' '}
            Also: <code>$top</code> and <code>$orderby</code> aren't supported inside this expand.
          </MessageBarBody>
        </MessageBar>
      )}
      {isNested && !parentIsNN && !parentIsCollection && (
        <MessageBar layout="multiline" intent="info" style={{ marginBottom: 12 }}>
          <MessageBarBody>
            You're inside a <strong>single-valued (N:1)</strong> expand. All cardinalities can be nested here{' '}
            (e.g. the classic <code>tasks → contact → account → systemuser</code> chain in the docs).
          </MessageBarBody>
        </MessageBar>
      )}
...
function NavPicker({
  ...
  const stagedIsCol = stagedNav?.cardinality === 'OneToMany' || stagedNav?.cardinality === 'ManyToMany';
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 0 }}>
                    ...
                    <span style={{ fontSize: 11, color: tokens.colorNeutralForeground3 }}>
                      ...
                      <span style={{ marginLeft: 8 }}>
                        {n.cardinality === 'ManyToMany' ? '(N:N — no nesting)' : isCol ? '(collection)' : '(single)'}
                      </span>
                    </span>
                  </div>
                </Option>
        ...
        >
          ...
          <div style={{ flexGrow: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <span style={{ fontFamily: tokens.fontFamilyMonospace, fontWeight: 600 }}>{stagedNav.name}</span>
              <span style={{ color: tokens.colorNeutralForeground3, fontSize: 12 }}>
                → {stagedTarget?.displayName ?? stagedNav.targetEntity}
              </span>
              <Badge appearance="ghost" size="small">{stagedNav.cardinality}</Badge>
            </div>
            <div style={{ display: 'flex', gap: 4, marginTop: 6, flexWrap: 'wrap' }}>
              <Caption1 style={{ color: tokens.colorNeutralForeground3, marginRight: 6 }}>
                Inner clauses you'll be able to set:
              </Caption1>
              <Badge size="small" appearance="ghost">$select</Badge>
              <Badge size="small" appearance="ghost">$filter</Badge>
              {stagedIsCol && (
                <>
                  <Badge size="small" appearance="ghost">$orderby</Badge>
                  <Badge size="small" appearance="ghost">$top</Badge>
                </>
              )}
              {stagedNav.cardinality !== 'ManyToMany' && (
                <Badge size="small" appearance="ghost">$expand (nested ManyToOne)</Badge>
              )}
            </div>
          </div>
        </div>
