...
import {
  Input, SpinButton, Switch, Combobox, Option, Persona, Field, Caption1, Badge,
  Tag, TagPicker, TagPickerControl, TagPickerGroup, TagPickerInput, TagPickerList,
  TagPickerOption, useTagPickerFilter, MessageBar, MessageBarBody, tokens, mergeClasses, Spinner,
  ToggleButton, Link, Button,
  Accordion, AccordionItem, AccordionHeader, AccordionPanel,
} from '@fluentui/react-components';
import { DatePicker } from '@fluentui/react-datepicker-compat';
import { TimePicker } from '@fluentui/react-timepicker-compat';
import { Settings20Filled, Document20Regular, FormNew20Regular, FormNew20Filled, Code20Regular, Open20Regular } from '@fluentui/react-icons';
...
export function ActionParamForm({
  action, values, setValue, setValues,
  group = 'execute', themeMode = 'light',
  ...
  useEffect(() => {
    if (bodyMode !== 'json') {
      ...
      try { setJsonText(JSON.stringify(values, null, 2)); } catch { /* ignore */ }
      return;
    }
    try { setJsonText(JSON.stringify(values, null, 2)); setJsonError(null); }
    ...
    return (
      <div>
        ...
        <MessageBar layout="multiline" intent="info" style={{ maxWidth: 720 }}>
          <MessageBarBody>
            <code>{action.name}</code> takes no parameters. Click <strong>Execute</strong> to run it.
          </MessageBarBody>
        </MessageBar>
      </div>
    );
  ...
  const missingRequired = action.parameters.filter(p => p.required).filter(p => isEmpty(values[p.name]));
  ...
  return (
    <div>
      <PaneHead
        ...
        group={group}
      >
        <Badge appearance="ghost">{action.parameters.filter(p => !isEmpty(values[p.name])).length} of {action.parameters.length} set</Badge>
        {missingRequired.length > 0 && (
          <Badge appearance="tint" color="danger">{missingRequired.length} required unset</Badge>
        )}
        {learnUrl && <DocsLink href={learnUrl} />}
          ...
          return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, maxWidth: 880 }}>
              ...
              {requiredParams.length === 0 && optionalParams.length > 0 && (
                <Caption1 style={{ color: tokens.colorNeutralForeground3, fontStyle: 'italic', padding: '4px 2px' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxWidth: 880 }}>
      <MessageBar layout="multiline" intent="info">
        <MessageBarBody>
          Hand-craft the request body as JSON. Useful when the metadata-driven
          form can't express the shape — e.g. Int32 params whose values map to
          documented enums, EntitySpecific params that should be{' '}
          <code>{'{ "@odata.id": "<set>(<id>)" }'}</code> references instead of
          inline instances, or Custom API params with domain-specific shapes.
          Refer to the MS Learn page for the canonical body.
        </MessageBarBody>
      </MessageBar>
      <div
        style={{
...
// ──────────────────────────────────────────────────────────────
function ParamRow({
  action, param, value, onChange, styles,
}: {
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2, paddingTop: 4 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ fontFamily: tokens.fontFamilyMonospace, fontWeight: 600 }}>{param.name}</span>
          {param.required && <Badge appearance="tint" color="danger" size="extra-small">req</Badge>}
        </div>
        <Caption1 style={{ color: tokens.colorNeutralForeground3, fontSize: 10 }}>
          <span style={{ color: tokens.colorBrandForeground2, fontFamily: tokens.fontFamilyMonospace }}>{param.type}</span>
...
// ──────────────────────────────────────────────────────────────
function ParamInput({ action, param, value, onChange }: {
  /**
  ...
  switch (param.type) {
    case 'Edm.String':
      return <Input size="small" value={(value as string) ?? ''} onChange={(_, d) => onChange(d.value)} placeholder="value" />;
    case 'Edm.Guid':
      return <Input size="small" value={(value as string) ?? ''} onChange={(_, d) => onChange(d.value)} placeholder="00000000-0000-0000-0000-000000000000" style={{ fontFamily: tokens.fontFamilyMonospace }} />;
        ...
        <SpinButton
          ...
          onChange={(_, d) => {
            const raw = d.value != null
              ? d.value
              : (d.displayValue != null ? Number(d.displayValue) : NaN);
            if (Number.isFinite(raw)) onChange(Math.trunc(raw));
          }}
        />
      );
    case 'Edm.Decimal': case 'Edm.Double':
      return <SpinButton size="small" value={Number(value ?? 0)} step={0.01} onChange={(_, d) => onChange(d.value ?? Number(d.displayValue ?? 0))} />;
    case 'Edm.Boolean':
      return <Switch checked={value === true} onChange={(_, d) => onChange(d.checked)} label={value === true ? 'true' : 'false'} />;
    case 'Edm.DateTimeOffset':
      return <DateTimeInput value={(value as string) ?? ''} onChange={onChange} />;
    case 'OptionSetValue':
      return <OptionSetInput param={param} value={value as number | undefined} onChange={onChange} />;
    ...
    case 'EntityReference':
      ...
      return <EntityReferenceInput value={value as { id: string; entityType: string } | undefined} onChange={onChange} param={param} polymorphic />;
    case 'EntitySpecific':
      ...
      return <EntitySpecificInput param={param} value={value as Record<string, unknown> | undefined} onChange={onChange} />;
    case 'EntityCollection':
      return <EntityCollectionInput value={(value as Array<{ id: string; entityType: string }>) ?? []} onChange={onChange} param={param} />;
    case 'ComplexType':
      return <ComplexTypeInput param={param} value={value as Record<string, unknown> | undefined} onChange={onChange} />;
    case 'Edm.Binary':
      return <Input size="small" value={(value as string) ?? ''} onChange={(_, d) => onChange(d.value)} placeholder="base64-encoded bytes" style={{ fontFamily: tokens.fontFamilyMonospace }} />;
  }
}
...
function DateTimeInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <span style={{ display: 'inline-flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
      <DatePicker size="small" value={valid} onSelectDate={write} placeholder="Date…" formatDate={(d) => d ? d.toLocaleDateString() : ''} allowTextInput style={{ minWidth: 130 }} />
      <TimePicker size="small" freeform selectedTime={valid} onTimeChange={(_, d) => writeTime(d.selectedTime)} placeholder="Time…" style={{ minWidth: 100 }} hourCycle="h23" />
    </span>
  );
}

// ── OptionSet combobox ──
function OptionSetInput({ param, value, onChange }: { param: ActionParam; value: number | undefined; onChange: (v: number) => void }) {
    ...
    >
      {options.map(o => (
        <Option key={o.value} value={String(o.value)} text={o.label}>
          {o.label}
          <span style={{ color: tokens.colorNeutralForeground3, fontFamily: tokens.fontFamilyMonospace, fontSize: 10, marginLeft: 6 }}>· {o.value}</span>
        </Option>
      ))}
    </Combobox>
  );
...
function StringArrayInput({ value, onChange, mono }: { value: string[]; onChange: (v: string[]) => void; mono?: boolean }) {
  return (
    <span style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
      ...
      <Input
        ...
        style={{ width: 160, fontFamily: mono ? tokens.fontFamilyMonospace : tokens.fontFamilyBase }}
        onKeyDown={(e) => {
          if (e.key === 'Enter') {
            const v = (e.target as HTMLInputElement).value.trim();
            if (v) { onChange([...value, v]); (e.target as HTMLInputElement).value = ''; }
            e.preventDefault();
          }
        }}
      />
...
function NumberArrayInput({ value, onChange }: { value: number[]; onChange: (v: number[]) => void }) {
  return (
    <span style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
      {value.map((v, i) => (
        <Tag key={i} shape="rounded" dismissible onClick={() => onChange(value.filter((_, j) => j !== i))}>
          {String(v)}
        </Tag>
      ))}
      <Input
        ...
        onKeyDown={(e) => {
          if (e.key === 'Enter') {
            ...
            if (Number.isFinite(v)) { onChange([...value, v]); (e.target as HTMLInputElement).value = ''; }
            e.preventDefault();
          }
        }}
      />
...
// no fixture catalog.
function EntityReferenceInput({ value, onChange, param, polymorphic }: {
  ...
  return (
    <span style={{ display: 'flex', flexDirection: 'column', gap: 6, minWidth: 0 }}>
      ...
      {!polymorphic && (
        <Badge appearance="ghost" style={{ fontFamily: tokens.fontFamilyMonospace, alignSelf: 'flex-start' }}>
      ...
      <Caption1 style={{ color: tokens.colorNeutralForeground3, fontSize: 10 }}>
        Emitted as <code style={{ fontFamily: tokens.fontFamilyMonospace }}>@odata.type=Microsoft.Dynamics.CRM.{entityType}</code>
      </Caption1>
    </span>
  );
}
...
function EntityCollectionInput({ value, onChange, param }: {
  ...
  if (!entityType) {
    return (
      <MessageBar layout="multiline" intent="warning">
        <MessageBarBody>This collection parameter has no entity type declared in CSDL.</MessageBarBody>
      </MessageBar>
    );
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, minWidth: 0 }}>
      ...
      {value.length > 0 && (
        <div style={{
          ...
          padding: 6, border: `1px solid ${tokens.colorNeutralStroke2}`,
          borderRadius: tokens.borderRadiusMedium,
        }}>
          {value.map(v => (
            <div key={v.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Persona size="extra-small" name={v.id} avatar={{ color: 'colorful' }} />
              <code style={{ flex: 1, minWidth: 0, fontFamily: tokens.fontFamilyMonospace, fontSize: 11, color: tokens.colorNeutralForeground3 }}>
                /{tbl?.entitySetName ?? entityType}({v.id})
              </code>
              <Button size="small" appearance="subtle" onClick={() => onRemove(v.id)}>Remove</Button>
            </div>
          ))}
        </div>
      )}
...
// ──────────────────────────────────────────────────────────────
function EntitySpecificInput({ param, value, onChange }: {
  param: ActionParam;
  ...
  if (!tbl) {
      ...
      return (
        <div style={{
          ...
          <Caption1 style={{ color: tokens.colorNeutralForeground3 }}>
            Loading <code>{entityType}</code> metadata from Dataverse — typed form will appear shortly…
          </Caption1>
        </div>
      );
    }
    return (
      <MessageBar layout="multiline" intent="warning">
        <MessageBarBody>
          No metadata for entity type <code>{entityType}</code> — the entity isn't available in this environment.
          For now, edit the JSON inline:
        </MessageBarBody>
        <Input
          size="small"
          ...
          onChange={(_, d) => {
            ...
            } catch { /* ignore — user is mid-typing */ }
          }}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
        ...
        <Caption1 style={{ color: tokens.colorNeutralForeground3 }}>new entity instance · @odata.type discriminator added automatically</Caption1>
      </div>
      <FieldSetEditor
        table={entityType}
        values={v}
...
function ComplexTypeInput({ param, value, onChange }: {
  param: ActionParam;
  value: Record<string, unknown> | undefined;
    ...
    return (
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
          <Document20Regular style={{ width: 14, height: 14, color: tokens.colorNeutralForeground3 }} />
          <strong style={{ fontSize: 11 }}>{param.complexType?.name ?? param.name}</strong>
          <Caption1 style={{ color: tokens.colorNeutralForeground3 }}>complex type · no field schema in mock — edit JSON</Caption1>
        </div>
        <Input
          size="small"
          value={JSON.stringify(v)}
          onChange={(_, d) => {
            ...
            } catch { /* mid-typing — drop until valid */ }
          }}
        ...
        <Field
          ...
          label={
            <span style={{ fontFamily: tokens.fontFamilyMonospace, fontSize: 11 }}>
              ...
              {f.required && <Badge appearance="tint" color="danger" size="extra-small" style={{ marginLeft: 6 }}>req</Badge>}
            </span>
          }
          hint={f.description}
        >
