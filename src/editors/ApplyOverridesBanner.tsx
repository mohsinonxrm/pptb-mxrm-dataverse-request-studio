 ...
 */
export function ApplyOverridesBanner({
  clause,
}: {
  clause: '$select' | '$filter' | '$orderby' | '$expand' | '$count';
}) {
  ...
  return (
    <MessageBar
      layout="multiline"
      intent={isOrderby ? 'error' : 'warning'}
      ...
      <MessageBarBody>
        ...
        {isOrderby ? (
          <>
            ...
            <em> "The query node SingleValueOpenPropertyAccess is not supported."</em> Remove your{' '}
            <code>$orderby</code>
            entries, or disable <code>$apply</code> if you need server-side ordering.
          </>
        ) : (
          <>
            <code>$apply</code> takes over the response shape — only <code>$top</code> remains
            alongside it. Output columns come from your groupby + aggregate aliases; row-level
            filtering moves into a<code>filter(…)/groupby(…)</code> stage. Disable{' '}
            <code>$apply</code> to use {clause} again.
          </>
        )}
      </MessageBarBody>
    </MessageBar>
