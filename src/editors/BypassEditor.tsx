...
import {
  Field, Radio, RadioGroup, Input, Switch, Caption1, Badge, Button,
...
export function BypassEditor({ value, onChange, group = 'write' }: BypassEditorProps) {
  ...
  const addStepId = () => {
    ...
    if (!v) return;
    if (value.stepIds.includes(v)) { setNewStepId(''); return; }
    patch({ stepIds: [...value.stepIds, v] });
    setNewStepId('');
  };
  const removeStepId = (i: number) =>
    patch({ stepIds: value.stepIds.filter((_, j) => j !== i) });
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18, maxWidth: 720 }}>
        ...
        <section>
          ...
          <Caption1 style={{ color: tokens.colorNeutralForeground2, display: 'block', marginBottom: 8 }}>
            Emits <code>MSCRM.BypassBusinessLogicExecution</code> with the chosen
            scope. Requires the <code>prvBypassCustomBusinessLogic</code> privilege —
            by default only System Administrator has it.
          </Caption1>
          <RadioGroup value={value.businessLogic} onChange={(_, d) => setMode(d.value as BypassBusinessLogicMode)} layout="vertical">
            <Radio value="none"  label={<RadioLabel title="None"           sub="Default — all custom logic runs." />} />
            <Radio value="sync"  label={<RadioLabel title="Sync only"      sub="Bypass synchronous plug-ins/workflows. Reduces per-record latency on bulk operations." />} />
            <Radio value="async" label={<RadioLabel title="Async only"     sub="Bypass async logic (system jobs). Lets bulk operations complete without queueing thousands of async ops." />} />
            <Radio value="both"  label={<RadioLabel title="Both"           sub="Skip both sync and async logic. Most aggressive option." />} />
            <Radio value="steps" label={<RadioLabel title="Specific steps" sub="Bypass only the named plug-in step registrations (GUIDs). Useful when one plug-in causes the slowdown." />} />
          </RadioGroup>

          {value.businessLogic === 'sync' && (
            <div style={{ marginTop: 10, padding: '8px 12px', borderRadius: tokens.borderRadiusMedium, background: tokens.colorNeutralBackground2 }}>
              <Switch
                ...
                label={
                  <span style={{ fontSize: 12 }}>
                    ...
                    <Caption1 style={{ display: 'block', color: tokens.colorNeutralForeground3 }}>
                      Off: emit modern header. On: emit legacy (requires <code>prvBypassCustomPlugins</code> privilege).
                    </Caption1>
                  </span>
                }
              />
          ...
          {value.businessLogic === 'steps' && (
            <div className={s.inlineCard} style={{ padding: 12, marginTop: 10 }}>
              <strong style={{ fontSize: 12, display: 'block', marginBottom: 6 }}>Plug-in step IDs</strong>
              <Caption1 style={{ color: tokens.colorNeutralForeground2, display: 'block', marginBottom: 8 }}>
                One GUID per row. Resolve them via{' '}
                <code>GET /sdkmessagefilters?$select=sdkmessagefilterid&amp;$filter=primaryobjecttypecode eq '&lt;table&gt;' and sdkmessageid/name eq '&lt;Op&gt;'&amp;$expand=sdkmessagefilterid_sdkmessageprocessingstep($select=name)</code>.
              </Caption1>
              <Field>
                <div style={{ display: 'flex', gap: 8 }}>
                  <Input
                    ...
                    style={{ flex: 1 }}
                    onKeyDown={(e) => { if (e.key === 'Enter') addStepId(); }}
                  />
                  <Button icon={<Add20Regular />} size="small" onClick={addStepId} disabled={!newStepId.trim()}>
                    Add
                  </Button>
                </div>
              </Field>
                ...
                <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {value.stepIds.map((id, i) => (
                    <div key={i} style={{
                      ...
                      padding: '4px 8px', background: tokens.colorNeutralBackground1,
                      borderRadius: tokens.borderRadiusSmall,
                    }}>
                      <span style={{ flex: 1 }}>{id}</span>
                      <Button icon={<Delete20Regular />} appearance="subtle" size="small" onClick={() => removeStepId(i)} />
                    </div>
                  ))}
                </div>
              )}
              ...
              {!stepErr && stepWarn && (
                <MessageBar layout="multiline" intent="warning" style={{ marginTop: 8 }}>
                  <MessageBarBody>
                    ...
                    <code> BypassBusinessLogicExecutionStepIdsLimit </code> OrgDbOrgSettings tweak (max {STEP_IDS_MAX_LIMIT}).
                  </MessageBarBody>
                </MessageBar>
              )}
            </div>
        ...
        <section>
          ...
          <Caption1 style={{ color: tokens.colorNeutralForeground2, display: 'block', marginBottom: 8 }}>
            Emits <code>MSCRM.SuppressCallbackRegistrationExpanderJob</code>. No
            privilege required, but flow owners are <strong>not notified</strong>
          ...
          {value.suppressFlows && (
            <MessageBar layout="multiline" intent="warning" style={{ marginTop: 8 }}>
              <MessageBarBody>
                Use only when you've confirmed a backlog of <code>CallbackRegistration
                Expander Operation</code> jobs (operationtype 79 in
                <code> asyncoperation</code>). Communicate with flow owners before
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <div style={{
      ...
      color: tokens.colorNeutralForeground3, marginBottom: 6,
    }}>{children}</div>
  );
}
