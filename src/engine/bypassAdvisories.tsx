...
import {
  STEP_IDS_DEFAULT_LIMIT, STEP_IDS_MAX_LIMIT,
  PRV_BYPASS_BUSINESS_LOGIC, PRV_BYPASS_CUSTOM_PLUGINS,
} from './bypassHeaders';

const LEARN_BYPASS_LOGIC = 'https://learn.microsoft.com/en-us/power-apps/developer/data-platform/bypass-custom-business-logic';
const LEARN_BYPASS_FLOWS = 'https://learn.microsoft.com/en-us/power-apps/developer/data-platform/bypass-power-automate-flows';
...
export function detectBypassAdvisories(b: BypassOptions, focusNode = 'bypass'): Advisory[] {
  ...
  if (b.businessLogic !== 'none') {
    const privilege = b.useLegacyHeader && b.businessLogic === 'sync'
    ...
    out.push(adv.warn(
      ...
      <span>
        This request requires the <code>{privilege.name}</code> privilege on
        the caller. {privilege.description}
      </span>,
      focusNode,
      LEARN_BYPASS_LOGIC,
    ));
      ...
      if (count === 0) {
        ...
        out.push(adv.warn(
          ...
          <span>
            ...
            <code> BypassBusinessLogicExecutionStepIdsLimit </code> via the
    ...
    if (b.useLegacyHeader && b.businessLogic === 'sync') {
      out.push(adv.info(
        ...
        <span>
          The newer <code>MSCRM.BypassBusinessLogicExecution: CustomSync</code>{' '}
          is the recommended replacement. Both wire effects are equivalent for
          sync-only bypass.
        </span>,
        focusNode,
        LEARN_BYPASS_LOGIC,
  ...
  if (b.suppressFlows) {
    out.push(adv.warn(
      ...
      <span>
        Flow owners are <strong>not notified</strong> when their logic is
        bypassed. Communicate with them before running this against production
        data — child flows may still fire via other triggers later.
      </span>,
      focusNode,
      LEARN_BYPASS_FLOWS,
    ));
...
function labelForMode(b: BypassOptions): string {
  switch (b.businessLogic) {
    ...
    case 'both':  return 'sync + async';
    case 'steps': return `${b.stepIds.filter(Boolean).length} specific plug-in step(s) of`;
    case 'none':
    default:      return '';
  }
}
