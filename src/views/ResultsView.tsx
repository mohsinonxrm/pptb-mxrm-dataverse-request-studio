...
export function ResultsView({
  ...
  if (!result) {
    return (
      <div className={s.emptyState}>
        ...
        <Caption1>
          Click <strong>Execute</strong> in the URL bar to run the request.
        </Caption1>
      </div>
    );
  }
  ...
  const serverTotal =
    typeof env?.['@odata.count'] === 'number' && env['@odata.count'] >= 0
      ? env['@odata.count']
      : undefined;
  ...
  if (result.outcome === 'empty') {
    return (
      <div
        style={{ display: 'flex', flexDirection: 'column', gap: 8, height: '100%', minHeight: 0 }}
      >
        <TabList
          selectedValue={tab}
...
function DetailPane({ body, table }: { body: unknown; table: string }) {
  ...
  return <RecordDetailCard record={rec} entityLogical={table} level={0} />;
}
...
function ErrorState({ result }: { result: ExecResult }) {
  ...
  const advice =
    result.status === 401
      ? 'Sign in to refresh your access token, then re-run.'
      : result.status === 403
        ? 'Caller is missing a required privilege. Check role assignments.'
        : result.status === 404
          ? 'Record (or entity) not found. Verify the GUID and entity set.'
          : 'Inspect the JSON body and headers tabs for details.';
  const kind: 'success' | 'danger' | 'warning' =
    result.status >= 400 ? 'danger' : result.status >= 300 ? 'warning' : 'success';
  return (
